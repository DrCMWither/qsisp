...
pub fn normalize_program(exprs: &[Expr], kw: &KeywordMap) -> Vec<Expr> {
    exprs.iter().map(|e| normalize_expr(e, kw)).collect()
}

fn normalize_expr(expr: &Expr, kw: &KeywordMap) -> Expr {
    match expr {
        Expr::Number(n) => Expr::Number(*n),
        Expr::String(s) => Expr::String(s.clone()),
        Expr::Symbol(s) => Expr::Symbol(s.clone()),
        Expr::List(items) => normalize_list(items, kw),
    }
}

fn normalize_list(items: &[Expr], kw: &KeywordMap) -> Expr {
    if items.is_empty() {
        return Expr::List(vec![]);
    }

    let head = match &items[0] {
        Expr::Symbol(s) => Expr::Symbol(kw.canonicalize(s)),
        other => normalize_expr(other, kw),
    };
        ...
        Some("define") => {
            if items.len() >= 2 {
                match &items[1] {
                    ...
                    Expr::List(sig) => {
                        ...
                        if let Some(first) = sig.first() {
                            match first {
                                ...
                                other => nsig.push(normalize_expr(other, kw)),
                            }
                        }
                    ...
                    other => out.push(normalize_expr(other, kw)),
                }
            }
            for arg in items.iter().skip(2) {
                out.push(normalize_expr(arg, kw));
            }
        }
                        ...
                        let nbindings = bindings
                            ...
                            .map(|b| match b {
                                Expr::List(pair) if pair.len() == 2 => {
                                    ...
                                    let value = normalize_expr(&pair[1], kw);
